...
type AuditLogAction =
  ...
  | "FORM_ARCHIVED"
  | "FORM_RESTORED";
type AuditLogTargetType = "user" | "form";
...
class AdminService {
  ...
  private async scalarCount(table: "users" | "forms" | "formSubmissions", where?: SQL) {
    ...
    return (
      (await db.select({ total: count() }).from(formSubmissionsTable).where(where))[0]?.total ?? 0
    );
  }

  private async createTransactionalAuditLog(
  ...
  public async getDashboard() {
    ...
    const metrics = {
      ...
      submissionsLast24h: await this.scalarCount(
        "formSubmissions",
        sql`${formSubmissionsTable.submittedAt} >= ${last24h}`,
      ),
      submissionsLast7d: await this.scalarCount(
        "formSubmissions",
        sql`${formSubmissionsTable.submittedAt} >= ${last7d}`,
      ),
  ...
  public async listUsers(input: ListAdminUsersInput) {
    const { page, pageSize, search, role, status } =
      await listAdminUsersInputSchema.parseAsync(input);
    const filters: SQL[] = [];
    if (search) {
  ...
  public async getUserDetail(input: GetAdminUserDetailInput) {
    ...
    const publishedFormCount = await this.scalarCount(
      "forms",
      and(eq(formsTable.createdBy, userId), eq(formsTable.status, "published")),
    );
    ...
    return {
      ...
      formCount,
      publishedFormCount,
      submissionCount: submissionRow?.total ?? 0,
      recentForms,
  ...
  public async updateUserRole(input: UpdateAdminUserRoleInput) {
    ...
    return db.transaction(async (tx) => {
      ...
      if (previousRole === "admin" && role === "user") {
        ...
        if ((adminCountRow?.total ?? 0) <= 1) throw new Error("Cannot demote the last admin");
      }
      ...
      if (!updated) throw new Error(`User With ${userId} Not Found`);
      await this.createTransactionalAuditLog(tx, {
        actorUserId,
  ...
  public async updateUserStatus(input: UpdateAdminUserStatusInput) {
    const { actorUserId, userId, status } =
      await updateAdminUserStatusInputSchema.parseAsync(input);
    ...
    return db.transaction(async (tx) => {
      ...
      if (currentRole === "admin" && status === "suspended") {
        const [activeAdminCountRow] = await tx
          .select({ total: count() })
          .from(usersTable)
          .where(and(eq(usersTable.role, "admin"), eq(usersTable.status, "active")));
        if ((activeAdminCountRow?.total ?? 0) <= 1)
  ...
  public async listForms(input: ListAdminFormsInput) {
    const { page, pageSize, search, status, visibility, creatorId } =
      await listAdminFormsInputSchema.parseAsync(input);
    const filters: SQL[] = [];
    if (search) {
      const searchFilter = or(
        ilike(formsTable.title, `%${search}%`),
        ilike(formsTable.slug, `%${search}%`),
        ilike(usersTable.email, `%${search}%`),
      );
      if (searchFilter) filters.push(searchFilter);
    }
    ...
    return {
      rows: rows.map((row) => ({
        ...row,
        status: formStatusSchema.parse(row.status),
        visibility: formVisibilitySchema.parse(row.visibility),
      })),
  ...
  public async getFormDetail(input: GetAdminFormDetailInput) {
    ...
    const responseCount = await this.scalarCount(
      "formSubmissions",
    ...
    return {
      form: {
        ...form,
        status: formStatusSchema.parse(form.status),
        visibility: formVisibilitySchema.parse(form.visibility),
      },
      fields,
      responseCount,
      recentSubmissions,
  ...
  private async moderateForm(
    formId: string,
    actorUserId: string,
    nextStatus: "draft" | "archived",
    action: "FORM_FORCE_UNPUBLISHED" | "FORM_ARCHIVED" | "FORM_RESTORED",
  ) {
    return db.transaction(async (tx) => {
      const [form] = await tx
        .select({
      ...
      if (previousStatus === nextStatus)
        return { id: formId, status: previousStatus, changed: false };
      if (action === "FORM_FORCE_UNPUBLISHED" && previousStatus !== "published")
        return { id: formId, status: previousStatus, changed: false };
      if (action === "FORM_RESTORED" && previousStatus !== "archived")
        return { id: formId, status: previousStatus, changed: false };
      await tx
        .update(formsTable)
        .set({ status: nextStatus, publishedAt: null })
        .where(eq(formsTable.id, formId));
      await this.createTransactionalAuditLog(tx, {
        actorUserId,
        action,
        ...
        metadata: {
          ...
          previousPublishedAt: form.publishedAt?.toISOString() ?? null,
        },
      });
  ...
  public async listSubmissions(input: ListAdminSubmissionsInput) {
    const { page, pageSize, formId, creatorId } =
      await listAdminSubmissionsInputSchema.parseAsync(input);
    const filters: SQL[] = [];
